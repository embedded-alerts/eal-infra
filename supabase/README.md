# Supabase overlays for embedded-alerts
...
**`embedded_alerts`**. Every migration in these overlays must stay inside `embedded_alerts`; the first migration creates it and
...
- **Authority does not move.** Portable SQL and JSON Schema stay in `embedded-alerts/eal-lib-core` (lib-core tier).
