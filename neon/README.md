...
| Path | Purpose |
| --- | --- |
| `neon/embedded-alerts-prod/terraform/` | pinned `kislerdm/neon` provider + lock file; project `embedded-alerts-prod` in `aws-us-east-2` with `prevent_destroy` |
...
State: **planned**. No embedded-alerts Neon organization or project has been provider-verified yet, so no ID in this repo is
...
- Migrations are authored in `embedded-alerts/eal-lib-core` and applied by `dpm`; this repository is not a migration authority.
